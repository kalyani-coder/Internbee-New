...
export default function ListofShortlist() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(`https://backend.internsbee.com/api/applyInternship/shortlisted/${id}`);
              ...
              <tbody>
                {shortlistData.map((shortlist) => (
                  <tr key={shortlist._id}>
                    <td className="border px-4 py-2">{shortlist.InternName}</td>

                    <td className="border px-4 py-2">{shortlist.InternEmail}</td>
                    <td className="border px-4 py-2">{shortlist.InternNumber}</td>


                    <td className="border px-4 py-2">{shortlist.job_Description}</td>
                    ...
                    <td className="border px-4 py-2">{shortlist.job_Title}</td>
                    <td className="border px-4 py-2">{shortlist.appliedDate}</td>
                  </tr>
                ))}
              </tbody>
            </table>
