...
const SubscriptionAnnually = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch('http://localhost:8000/api/adminannuallypackage');
    ...
    const savePackage = async () => {
        try {
            const response = await fetch(`http://localhost:8000/api/adminannuallypackage/${packageData._id}`, {
                method: 'PATCH',
                headers: {
                ...
                body: JSON.stringify({
                    ...
                    annuallyPackage_Price: editedPackage.annuallyPackage_Price,
            ...
            if (response.ok) {
                ...
                setPackageData((prevData) => ({ ...prevData, ...editedPackage }));
                                    ...
                                    <div className="p-4  w-full">
                                        <div className="h-full p-6 rounded-lg border-2 border-amber-300 flex flex-col relative overflow-hidden">
                                            ...
                                            <h1 className="text-5xl text-gray-900 leading-none flex items-center pb-4 mb-4 border-b border-amber-300">
                                                <span>{`₹${editedPackage.annuallyPackage_Price}`}</span>
                                            ...
                                            <p className="flex items-center text-gray-600 mb-2">
                                                ...
                                                </span>{packageData.internship_enquiry}
                                            </p>
                                            ...
                                            <p className="flex items-center text-gray-600 mb-2">
                                                ...
                                                </span>{packageData.verified_appication}

                                            </p>

                                            <p className="flex items-center text-gray-600 mb-2">
                                                <span className="w-4 h-4 mr-2 inline-flex items-center justify-center bg-gray-400 text-white rounded-full flex-shrink-0">
                                                    <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" className="w-3 h-3" viewBox="0 0 24 24">                                                     <path d="M20 6L9 17l-5-5"></path>
                                                    </svg>
                                                </span>{packageData.resume_view}
                                            </p>
                                            ...
                                            <p className="flex items-center text-gray-600 mb-6">
                                                ...
                                                </span>{packageData.dedicated_crm}
                                            </p>

                                            <button  onClick={openModal}
                                            className="flex items-center mt-auto text-black bg-amber-300 border-0 py-2 px-4 w-full focus:outline-none hover:bg-gray-500 rounded">Edit Package
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-gray-700 bg-opacity-50 flex items-center justify-center">
                    <div className="bg-white p-8 max-w-md w-full rounded-md">
                        ...
                        <input
                            type="text"
                            name="annuallyPackage_Price"
                            value={editedPackage.annuallyPackage_Price}
