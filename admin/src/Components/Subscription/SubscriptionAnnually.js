...
const SubscriptionAnnually = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('https://backend.internsbee.com/api/adminannuallypackage');
...
const savePackage = async () => {
    try {
        const response = await fetch(`https://backend.internsbee.com/api/adminannuallypackage/${monthlyPackage._id}`, {
            method: 'PATCH',
            headers: {
  ...
  <div className="container px-5 py-5 mx-auto">
    <div className="flex flex-col text-center w-full mb-20">
      <h1 className="sm:text-4xl text-3xl font-medium title-font mb-2 text-black">Annually Packages</h1>
      ...
      <div className="flex mx-auto border-2 border-amber-300 rounded overflow-hidden mt-6">
        ...
        <Link to={'/subscriptionannually'}>
          <button className={`py-1 px-4 text-black focus:outline-none  'border-b-2 border-indigo-600'`}>Annually</button>
        </Link>
      </div>
    </div>
        ...
        <div className="p-4 xl:w-full md:w-1/2 w-full">
          <div className="h-full p-6 rounded-lg border-2 border-amber-300 flex flex-col relative overflow-hidden">
            ...
            <h1 className="text-3xl sm:text-5xl text-black leading-none flex items-center pb-4 mb-4 border-b border-amber-300">
              <span>{`₹${monthlyPackage.annuallyPackage_Price}`}</span>
            ...
            <p className="flex items-center text-gray-600 mb-6">
              <span className="w-4 h-4 mr-2 inline-flex items-center justify-center bg-gray-400 text-white rounded-full flex-shrink-0">                     <svg fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" className="w-3 h-3" viewBox="0 0 24 24">
                <path d="M20 6L9 17l-5-5"></path>
              </svg>
